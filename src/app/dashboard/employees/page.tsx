...
export default async function EmployeesPage() {
  ...
  // Fetch face registrations (encrypted)
  const userIds = empList.map(e => e.id)
  const { data: faceRegs } = await admin
    .from('face_registrations')
    .select('user_id, face_data, face_data_encrypted, face_photo_url, face_photo_path')
    .in('user_id', userIds)

  const faceRegMap = new Map((faceRegs ?? []).map(r => [r.user_id, r]))

  // Merge: has_face = true if any face data exists (encrypted or plain)
  const mergedEmployees = empList.map(emp => {
    const reg = faceRegMap.get(emp.id)
    return {
      ...emp,
      face_data: reg?.face_data_encrypted ? 'encrypted' : (reg?.face_data ?? null),
      face_photo_path: reg?.face_photo_path ?? null,
      face_photo_url: reg?.face_photo_url ?? null,
      has_face_registration: !!(reg?.face_data_encrypted || reg?.face_data),
    }
  })
